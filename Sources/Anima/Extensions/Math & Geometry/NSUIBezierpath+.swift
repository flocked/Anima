...
//
    ...
    extension NSBezierPath {
        ...
        convenience init(roundedRect rect: CGRect, byRoundingCorners corners: NSRectCorner, cornerRadius radius: CGFloat) {
            ...
            move(to: topLeft.offset(x: 0, y: radius))
            appendArc(from: topLeft, to: topRight, radius: corners.contains(.topLeft) ? radius : 0)
            appendArc(from: topRight, to: bottomRight, radius: corners.contains(.topRight) ? radius : 0)
            appendArc(from: bottomRight, to: bottomLeft, radius: corners.contains(.bottomRight) ? radius : 0)
            appendArc(from: bottomLeft, to: topLeft, radius: corners.contains(.bottomLeft) ? radius : 0)
            close()
        }
