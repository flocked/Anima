...
extension NSUIView {
    ...
    var removeSuperview: NSUIView? {
        get { getAssociatedValue("removeSuperview") }
        set { setAssociatedValue(weak: newValue, key: "removeSuperview") }
    }
    
    func removeFromSuperviewIfNeeded() {
        if let removeSuperview = removeSuperview, superview === removeSuperview {
            removeFromSuperview()
        }
        removeSuperview = nil
    }
...
extension UIView {
    ...
    var preventingUserInteractionAnimations: Set<UUID> {
        ...
        set {
            ...
            } else if newValue.isEmpty, preventsUserInteractions {
